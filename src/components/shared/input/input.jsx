...
const Input = (props) => {
    ...
    const handleSubmit = (evt) => {
        ...
        const postsToday = ownPosts.slice(-5).filter(({createdAt}) => createdAt === today)
    ...
    return (
        <>
            <form onSubmit={handleSubmit} className="flex align-middle">
                {
                    withoutAvatar || (
                        <label htmlFor="large-input" className="m-4 text-sm font-medium text-gray-900 dark:text-gray-300">
            ...
            {limitPostExceeded &&
                <div id="toast-danger"
                    ...
                    <div className="ml-3 text-sm font-normal">Exceeded limit of posts today.</div>
                    <button type="button" onClick={() => setLimitPostExceeded(false)}

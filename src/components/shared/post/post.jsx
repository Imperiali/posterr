    ...
    return (
        <div className="py-3 sm:py-4 flex">
            ...
            <div className="flex flex-col ml-4 w-full">
                {originalUser &&
                    <a onClick={() => pathToProfile(originalUser)}
                ...
                <a onClick={() => pathToProfile(user)} className="text-gray-400 cursor-pointer">{user}</a>
                <p className="text-white">{message}</p>
                {originalPost && <Post {...originalPost}/>}
                {type !== 'replied' && <div className="flex text-blue-400 mt-4">
                    {postLiked
                        ? <button onClick={() => dislikePost({postUsername: user, currentUsername})}
                                  className='mr-4 text-pink-500'>Dislike</button>
                        : <button onClick={() => likePost({postUsername: user, currentUsername})}
                                  className='mr-4'>Like</button>}
                    <button onClick={handleRepost} className='mr-4'>Repost</button>
                    <button onClick={() => setRepliedPost(!repliedPost)} className='mr-4'>Reply</button>
                </div>}
                {
                    repliedPost && <Input withoutAvatar post={{user, message, originalUser, likes, type: 'replied'}} />
                }
            </div>
